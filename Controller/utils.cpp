...
namespace cocoDB {
    ...
    void cmd_wipe( array_t<string_t>& match, queue_t<cmd_t> list ){
        ...
        /*
        auto n=sql_list.raw().last(); while( n!=nullptr ){ 
            n->data.second.close(); n=n->next; 
        }   sql_list.erase();

        for( auto x: fs::read_folder( dir ) ) {
             fs::remove_file( path::join( dir, x ) );
        }    cb.emit( "+OK\r\n" );
        */
